...
const state = {
  ...
  slotImages: {},          // { slug: { name, dataUrl } } — Step 3 슬롯에 드롭된 이미지
...
async function loadStyles() {
  try {
    ...
    toast(`스타일 로드 실패: ${err.message}`, 'error');
  }
}
...
async function loadReferences() {
  try {
    ...
    // 레퍼런스 없어도 도구는 동작해야 한다 — 토스트만, 차단 X
...
async function callPlan() {
  ...
  try {
    ...
    renderPlanCards(data.plan);
    renderSlotCards(data.plan);
    showStep(1, 'done');
    showStep(2, 'unlock');
    showStep(3, 'unlock');
    showStep(4, 'unlock');
    ...
    toast('기획 완성. ChatGPT에 영문 프롬프트 붙여넣어 이미지를 받아오세요.', 'success', 3500);
  } catch (err) {
...
// ────────── 초기화 ──────────
async function init() {
  setupDropzone('product');
  setupDropzone('reference');
  setupPaste();
  bindEvents();
  await Promise.all([loadStyles(), loadReferences()]);
  await pingServer();
